...
Module.register("Carousel", {
    ...
    if (notification === "MODULE_DOM_CREATED") {
      ...
      if (
        this.config.keyBindings.enabled &&
          MM.getModules().filter((kb) => kb.name === "KeyBindings").length > 0
      ) {
        this.keyBindings = {
          ...this.keyBindings,
  ...
  moduleTransition (goToIndex = -1, goDirection = 0, goToSlide = undefined) {
    ...
    // Some modules like RTSPStream get into an odd state if you enable them when already enabled
    ...
    if (
      ...
      if (this.showPageIndicators) {
        const currPages = document.getElementsByClassName("carousel-current-page");
        if (currPages && currPages.length > 0) {
          for (let i = 0; i < currPages.length; i += 1) {
            currPages[i].classList.remove("carousel-current-page");
          }
        }
        document
          .getElementById(`sliderLabel_${this.currentIndex}`)
          .classList.add("carousel-current-page");
      }

      if (this.showPageControls) {
        const currBtns = document.getElementsByClassName("carousel-available");
        if (currBtns && currBtns.length > 0) {
          while (currBtns.length > 0) {
            currBtns[0].classList.remove("carousel-available");
          }
        }
        if (this.currentIndex !== resetCurrentIndex - 1) {
          ...
          document
            ...
            .classList.add("carousel-available");
        }
  ...
  updatePause (paused) {
    ...
    if (this.paused) carousel.classList.add("carousel-paused");
    else carousel.classList.remove("carousel-paused");
  },
