...
Module.register("Formula1", {
  ...
  getStyles() {
    return ["font-awesome.css", "Formula1.css"];
  },
  ...
  getTemplate() {
    return "templates\\formula1-standings.njk";
  },
  ...
  formatDateAndTime(dateTime) {
    ...
    if (dateTime.date && dateTime.time) {
      if (config.timeFormat === 24) {
        return moment(dateTime.date + "T" + dateTime.time).format("DD HH:mm");
      } else {
        return moment(dateTime.date + "T" + dateTime.time).format("DD hh:mm A");
      }
    }
    if (dateTime.date) {
      return moment(dateTime.date).format("DD") + waitingfortime;
    }
